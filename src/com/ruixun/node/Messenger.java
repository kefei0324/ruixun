...
import com.ruixun.node.udpconnector.UdpConnector;
...
public class Messenger implements Runnable {
	...
	private void deliverMessage(ClientMessage m) throws Exception {
		// System.out.println(this.hostThread.getName()+"
		// receive:"+StringUtil.convert(m.getData()));
		...
		/*
		 * ClientStatMachine csm = nodeStat.getClientStat(uuid); if(csm ==
		 * null){// csm = ClientStatMachine.newByClientTick(m); if(csm == null){
		 * return; } nodeStat.putClientStat(uuid, csm); }
		 * ArrayList<ServerMessage> smList = csm.onClientMessage(m); if(smList
		 * == null){ return; } for(int i = 0; i < smList.size(); i++){
		 * ServerMessage sm = smList.get(i); if(sm.getSocketAddress() ==
		 * null)continue; this.connector.send(sm); }
		 */

...
public class IMServerConsole implements Runnable {
	...
	private void onClean(String cmd, OutputStream response) throws Exception {
		...
		try {
			...
			response.write(("clean cmd format error: " + cmd
					+ ", should be like: clean-48, which means to clean stat machines who's last active time is more than 48 hours before now")
							.getBytes());
			response.flush();
			return;
		}
	...
	public static void main(String[] args) {
		...
		if (!"stop".equals(args[0]) && !"status".equals(args[0]) && !"gc".equals(args[0])
				&& !args[0].startsWith("uuid-") && !args[0].startsWith("clean-")) {
			System.out.println(
					"cmd: " + args[0] + " not found. should be: stop | status | gc | uuid-hexuuid | clean-hours");
			return;
		}
